...
function StepContent({ step, lesson }) {
  ...
  switch (step) {
    ...
    case 5: return (
      <div style={s.testHint}>
        Test tayyor — "Testga o'tish" tugmasini bosing
        <span style={{ fontSize: 14, color: '#555', fontWeight: 400, marginTop: 8, display: 'block' }}>
          7 ta savol · O'tish uchun 70% kerak
        </span>
      </div>
    );
...
const s = {
  ...
  navRow:          { position: 'fixed', bottom: '70px', left: 0, right: 0, display: 'flex', gap: 8,
                     padding: '12px 16px', background: 'rgba(240,244,255,0.88)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderTop: '1.5px solid rgba(255,255,255,0.9)' },
