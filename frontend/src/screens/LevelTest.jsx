...
export default function LevelTest() {
  ...
  const { user, markCompleted, lessons, setLessons } = useStore();
  ...
  useEffect(() => {
    api.getLesson(id).then((l) => {
      ...
      // Agar lessons store bo'sh bo'lsa yuklaymiz
      if (!lessons.length) {
        api.getLessons(l.track).then(setLessons);
      }
    });
  }, [id]);
  ...
  if (result) return (
    <div style={s.resultPage}>
      <div style={s.scoreCircle}>
        ...
        <div style={s.scoreLabel}>{result.correct}/{result.total} to'g'ri</div>
      </div>
      ...
      {result.passed ? (
        ...
        <div style={s.failBox}>
          ...
          <div style={s.failHint}>
            Yana {Math.ceil(questions.length * 0.7) - result.correct} ta to'g'ri javob kerak
          </div>
